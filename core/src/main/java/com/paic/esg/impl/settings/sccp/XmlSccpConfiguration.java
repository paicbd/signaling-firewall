...
public class XmlSccpConfiguration {
  ...
  SccpSettings sccp;
  private static final Logger logger = Logger.getLogger(XmlSccpConfiguration.class);
  ...
  private SccpAddress readSccpAddress(final Element sccpAddr, String routingIndicator) {
    ...
    int networkId = -1;
    if (!sccpAddr.getAttribute("networkId").isEmpty()){
      networkId = Integer.parseInt(sccpAddr.getAttribute("networkId"));
    }
    ...
    if (gtList != null && gtList.getLength() > 0) {
      ...
      return SccpRoutingAddress.convertToSccpAddress(ri, dpc, ssn, type, tt, es, np, nao, gtDigits, networkId);
    }
    return null;
  }
}
